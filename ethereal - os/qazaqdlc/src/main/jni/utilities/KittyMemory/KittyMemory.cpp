...
namespace KittyMemory {
    ...
    {
		std::string stringg = oxorany("proc/self/cmdline");
        const char *file = stringg.c_str();
		std::string rstr = oxorany("r");
		
        char cmdline[128] = {0};
        FILE *fp = fopen(file, rstr.c_str());
        if (!fp) {
    ...
    std::vector<ProcMap> getAllMaps()
    {
		...
		std::string stringg = oxorany("proc/self/maps");
        const char *file = stringg.c_str();
		std::string rstr = oxorany("r");
        char line[512] = {0};
    ...
    ProcMap getElfBaseMap(const std::string& name)
    {
        ...
        for (auto &it: maps)
        {
            ...
            if (strstr(it.pathname.c_str(), oxorany("/bin/linker"))) {
                retMap = it;
                break;
            }
