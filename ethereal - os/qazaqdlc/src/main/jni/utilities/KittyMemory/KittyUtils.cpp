...
namespace KittyUtils {
    ...
    {
        char *storage = getenv(oxorany("EXTERNAL_STORAGE"));
        return storage ? storage : oxorany("/sdcard");
    }
    ...
    int getAndroidVersion()
    {
        ...
        char buf[0xff] = { 0 };
        if (__system_property_get(oxorany("ro.build.version.release"), buf))
            ver = std::atoi(buf);
    ...
    int getAndroidSDK()
    {
        ...
        char buf[0xff] = { 0 };
        if (__system_property_get(oxorany("ro.build.version.sdk"), buf))
            sdk = std::atoi(buf);
    ...
    {
        static const std::string chars = oxorany("0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz");
